...
def title_tracks
  ...
  execute(<<-SQL)
    ...
    FROM
      albums
      JOIN
        tracks ON tracks.album = albums.asin and
...
def rock_superstars
  ...
  execute(<<-SQL)
    ...
    FROM
      albums
      JOIN
      styles ON styles.album = albums.asin AND
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, SUM(albums.price) / SUM(track_totals.track_count) as avg_price
    FROM
      styles
      JOIN
        (SELECT
          albums.asin AS album_id, COUNT(*) AS track_count
        FROM
          albums
          JOIN
            tracks ON tracks.album = albums.asin
        GROUP BY
          albums.asin) AS track_totals
      ON
        track_totals.album_id = styles.album
    JOIN
      albums
      ON
    ...
    LIMIT
      5
    -- ORDER BY
    --   albums.price / track_totals.track_count
  SQL
end
